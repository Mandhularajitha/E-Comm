...
function FilterPriceData (state , data) {
  ...
  return data.filter((item) => item.price >= state.filterByPrice)
}
...
const finaldata = (state,data,...allFunctions)=>{
  return allFunctions.reduce((acc,cur)=>cur(state,acc),data)
}

export {sortedData,FilterCategoryData,FilterPriceData,FilterRatingData,finaldata}
